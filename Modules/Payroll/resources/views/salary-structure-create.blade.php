...
@section('page-title', __("Create Salary Structure"))
...
@section('page-actions')
  <a href="{{ route('payroll.salary-structures') }}" class="bp-btn bp-btn-primary">
    <i class="fa-solid fa-arrow-left"></i> Back to Structures
  </a>
@endsection

@section('content')

  <form action="{{ route('payroll.salary-structures.store') }}" method="POST" id="structure-form">
    @csrf
      ...
      <div class="col-xl-8">
          ...
          <div class="bp-card-body">
            <div class="row g-3">
              <div class="col-md-4">
                ...
                <input type="text" class="bp-form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" required placeholder="e.g., Standard Staff">
                @error('name')
              ...
              <div class="col-md-4">
                ...
                <input type="text" class="bp-form-control @error('code') is-invalid @enderror" name="code" value="{{ old('code') }}" required placeholder="e.g., STD-STAFF" maxlength="30">
                @error('code')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              <div class="col-md-4">
                ...
                <select class="bp-form-select w-100 @error('is_active') is-invalid @enderror" name="is_active">
              ...
              <div class="col-12">
                ...
                <textarea class="bp-form-control @error('description') is-invalid @enderror" name="description" rows="2" placeholder="Brief description of this salary structure...">{{ old('description') }}</textarea>
                @error('description')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
        ...
        <div class="bp-card mb-4">
          ...
          <div class="bp-card-body p-0">
            ...
            <div class="p-3 text-center text-muted fs-13" id="no-components-msg">
              <i class="fa-solid fa-info-circle me-1"></i> No components added yet. Click "Add Component" to start building the salary structure.
            </div>
          </div>
        </div>
        ...
        <div class="bp-card">
          <div class="bp-card-footer text-end">
            <a href="{{ route('payroll.salary-structures') }}" class="bp-btn bp-btn-danger me-2"><i class="fa-solid fa-xmark me-1"></i>Cancel</a>
            <button type="submit" class="bp-btn bp-btn-success"><i class="fa-solid fa-check me-1"></i> Save Structure</button>
          </div>
        </div>
      </div>
      ...
      <div class="col-xl-4">
        <div class="bp-card bp-card-sticky">
          ...
          <div class="bp-card-body">
            ...
            <div id="preview-deductions">
              <div class="d-flex justify-content-between fs-13 mb-1 text-muted">
                <span>None</span>
...
$(document).ready(function () {
    ...
    function addComponentRow(data) {
        ...
        var row = '<tr data-index="' + idx + '">' +
            ...
            '</select></td>' +
            '<td>' +
            '<input type="number" class="bp-form-control bp-form-control-sm component-amount" name="components[' + idx + '][amount]" value="' + (data.amount || '0') + '" min="0" step="0.01" placeholder="Amount">' +
            '<input type="number" class="bp-form-control bp-form-control-sm component-percentage" name="components[' + idx + '][percentage]" value="' + (data.percentage || '0') + '" min="0" max="100" step="0.01" placeholder="%">' +
            '</td>' +
    ...
    function toggleNoComponentsMsg() {
        if ($('#components-body tr').length === 0) {
    ...
    function updatePreview() {
        ...
        $('#preview-basic').text(formatBDT(basic));
        $('#preview-total-earnings').text(formatBDT(totalEarnings));

        if (deductionsHtml === '') {
            deductionsHtml = '<div class="d-flex justify-content-between fs-13 mb-1 text-muted"><span>None</span><span>{{ currency_symbol() }} 0</span></div>';
        }
    ...
    // Toggle calc fields on change
    $(document).on('change', '.component-calc', function () {
        toggleCalcFields();
        updatePreview();
    });
