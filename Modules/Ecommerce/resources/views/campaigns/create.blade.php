...
@section('page-title', __('New Campaign'))
...
@section('content')
    ...
    <form action="{{ route('ecommerce.campaigns.store') }}" method="POST">
        @csrf
        <div class="bp-card">
            ...
            <div class="bp-card-footer text-end">
                <a href="{{ route('ecommerce.campaigns.index') }}" class="bp-btn bp-btn-danger"><i
                        class="fa-solid fa-xmark me-1"></i>{{ __('Cancel') }}</a>
                <button type="submit" class="bp-btn bp-btn-success">
                    <i class="fa-solid fa-save me-1"></i> {{ __('Save Campaign') }}
                </button>
            </div>
        </div>
    </form>
