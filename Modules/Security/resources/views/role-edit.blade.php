...
@section('content')
    ...
    <form action="{{ route('security.roles.update', $role->id) }}" method="POST">
        @csrf
        @method('PUT')
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="bp-form-control @error('name') is-invalid @enderror" name="name"
                            value="{{ old('name', $role->name) }}" required placeholder="e.g., Branch Manager">
                        @error('name')
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="bp-form-control @error('description') is-invalid @enderror"
                    ...
                    <table class="bp-table bp-permissions-table">
                        ...
                        <tbody>
                            ...
                            @foreach ($permissionGroups ?? [] as $group => $permissions)
                                @php
                                    $allStandard = array_filter($permissions, function ($p) use (
                                        $group,
                                        $standardActions,
                                    ) {
                                        return in_array(str_replace($group . '.', '', $p), $standardActions);
                                    });
                                    $allChecked =
                                        count($permissions) > 0 &&
                                        count(array_intersect($permissions, $checked)) === count($permissions);
                                @endphp
                                <tr data-module="{{ $group }}">
                                    ...
                                    @foreach ($standardActions as $action)
                                        <td class="text-center">
                                            ...
                                            @if (in_array($permName, $permissions))
                                                <input type="checkbox" class="bp-form-check perm-check" name="permissions[]"
                                                    value="{{ $permName }}"
                                                    {{ in_array($permName, $checked) ? 'checked' : '' }}>
                                            @else
                                    ...
                                    <td class="text-center">
                                        @php
                                            $otherPerms = array_filter($permissions, function ($p) use (
                                                $group,
                                                $standardActions,
                                            ) {
                                                return !in_array(str_replace($group . '.', '', $p), $standardActions);
                                        ...
                                        @foreach ($otherPerms as $perm)
                                            <div class="d-inline-block me-1">
                                                ...
                                                <label class="fs-11 text-muted"
                                                    for="perm_{{ str_replace('.', '_', $perm) }}">{{ ucfirst(str_replace($group . '.', '', $perm)) }}</label>
                                            </div>
                                        @endforeach
                                        @if (empty($otherPerms))
                                            <span class="text-muted">--</span>
                                    ...
                                    <td class="text-center"><input type="checkbox" class="bp-form-check module-select-all"
                                            {{ $allChecked ? 'checked' : '' }}></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <!-- Footer Actions -->
        <div class="text-end">
            <a href="{{ route('security.roles') }}" class="bp-btn bp-btn-danger me-2"><i
