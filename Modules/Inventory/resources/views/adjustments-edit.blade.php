...
@section('page-title', __('Edit Stock Adjustment'))
...
@section('breadcrumb')
    ...
    <span class="sep"><i class="fa-solid fa-chevron-right"></i></span>
    <a
        href="{{ route('inventory.adjustments.show', $adjustment) }}">{{ $adjustment->reference ?? 'ADJ-' . $adjustment->id }}</a>
    <span class="sep"><i class="fa-solid fa-chevron-right"></i></span>
    <span>Edit</span>
@endsection
...
@section('content')
    ...
    <form action="{{ route('inventory.adjustments.update', $adjustment) }}" method="POST" id="editAdjustmentForm">
        @csrf
        @method('PUT')
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select class="bp-form-select w-100 @error('type') is-invalid @enderror" name="type" required>
                            ...
                            <option value="addition"
                                {{ old('type', $adjustment->type) === 'addition' ? 'selected' : '' }}>Addition (+ stock)
                            </option>
                            <option value="subtraction"
                                {{ old('type', $adjustment->type) === 'subtraction' ? 'selected' : '' }}>Subtraction (−
                                stock)</option>
                        </select>
                        @error('type')
                        ...
                        <select class="bp-form-select w-100 @error('reason_id') is-invalid @enderror" name="reason_id"
                            ...
                            @foreach ($adjustmentReasons as $adjReason)
                                <option value="{{ $adjReason->id }}"
                                    {{ old('reason_id', $adjustment->reason_id) == $adjReason->id ? 'selected' : '' }}>
                                    {{ $adjReason->name }}</option>
                            @endforeach
                        </select>
                        @error('reason_id')
                            <div class="text-danger fs-12 mt-1">{{ $message }}</div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="bp-form-control" name="reference"
                            placeholder="Optional reference number" value="{{ old('reference', $adjustment->reference) }}">
                        @error('reference')
                            <div class="text-danger fs-12 mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-12">
                        ...
                        <textarea class="bp-form-control" name="notes" rows="3" placeholder="Optional notes about this adjustment...">{{ old('notes', $adjustment->notes) }}</textarea>
                        @error('notes')
                            <div class="text-danger fs-12 mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                                ...
                                @foreach ($adjustment->items as $i => $item)
                                    <tr class="item-row">
                                        ...
                                        <td>
                                            <select class="bp-form-select w-100 select2-search bp-select2-variant"
                                                name="items[{{ $i }}][variant_id]"
                                                data-old-variant="{{ $item->variant_id }}" disabled>
                                                <option value="">N/A</option>
                                            </select>
                                            @error('items.' . $i . '.variant_id')
                                                <div class="text-danger fs-12 mt-1">{{ $message }}</div>
                                        ...
                                        <td>
                                            <input type="number" class="bp-form-control"
                                                name="items[{{ $i }}][unit_cost]" min="0"
                                                step="0.01" placeholder="Cost" value="{{ $item->unit_cost }}">
                                            @error('items.' . $i . '.unit_cost')
                                                <div class="text-danger fs-12 mt-1">{{ $message }}</div>
                                            @enderror
                                        </td>
...
@push('scripts')
    <script>
        ...
        $(function() {
            var itemIndex = {{ old('items') ? count(old('items')) : $adjustment->items->count() }};
            ...
            // Restore variant dropdowns for existing items
