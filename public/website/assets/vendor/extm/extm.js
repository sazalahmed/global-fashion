	...
	$.fn.extend({
		extm: function extm(userOptions) {
			...
			function extmInit(options, imageElement) {
				...
				// NOTE: the storefront has a global `img { width:100%!important;
				// height:100%!important; object-fit:cover!important }` rule. Without
				// matching !important here it clobbers the clone's width, so the zoom
				// panel shows the whole image at 1:1 instead of a magnified region.
				// Reset height/object-fit too so the enlarged clone keeps aspect ratio.
				var fullSizeReset = 'position:relative;max-width:none !important;height:auto !important;object-fit:fill !important;';
				if (options.zoomSize) {
					var fullSizeImage = $('<img style="'+fullSizeReset+'width:'+options.zoomSize+'px !important;">'); //make a large clone and insert it
				}
				else if (options.zoomLevel !== 1) {
					var fullSizeImage = $('<img style="'+fullSizeReset+'width:'+(100*options.zoomLevel)+'% !important;">'); //make a large clone and insert it
				}
				else {
					var fullSizeImage = $('<img style="'+fullSizeReset+'">'); //make a large clone and insert it
				}
				if (options.imageSrc) {
				...
				(function(imageElement,zoomElement,fullSizeImage,overlayElement,smallHeight,smallWidth,offset, innerOverlayElement,options) {
					...
					// jQuery 3.x removed .load(handler) — must use .on('load', ...) now.
					// Also trigger immediately for cached images (the load event
					// fires before our handler is bound when the browser has the
					// image in its cache).
					var onFullSizeLoaded = function(){
					...
					fullSizeImage.on('load', onFullSizeLoaded);
					// Cached images fire 'load' before our handler is bound,
					// so check completeness and invoke directly when applicable.
					if (fullSizeImage[0] && fullSizeImage[0].complete && fullSizeImage[0].naturalWidth) {
						onFullSizeLoaded();
					}
