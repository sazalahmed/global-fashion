...
@section('title', __('Supplier Groups'))
@section('page-title', __('Supplier Groups'))
...
@section('content')
                    ...
                    <tbody>
                        @forelse($groups as $group)
                            <tr>
                                ...
                                <td class="fs-13">{{ $group->description ?? '' }}</td>
                                <td class="fw-600">{{ $group->suppliers_count }}</td>
                                <td><x-core::status-toggle :url="route('supplier-groups.toggle-status', $group->id)" :active="$group->is_active" /></td>
                                <td>
                                    @bpCanAny('suppliers.edit','suppliers.delete')
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            ...
                                            @endbpCan
                                            @if ($group->suppliers_count === 0)
                                                @bpCan('suppliers.delete')
                                                <li>
                                                    <form action="{{ route('supplier-groups.destroy', $group) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                        ...
                        @empty
                            <x-core::table.empty colspan="5" icon="fa-solid fa-user-group"
                                title="No supplier groups found." />
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <div class="modal fade" id="addGroupModal" tabindex="-1">
        <div class="modal-dialog">
            <form action="{{ route('supplier-groups.store') }}" method="POST">
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title fw-700">Add Supplier Group</h5>
    ...
    <div class="modal fade" id="editGroupModal" tabindex="-1">
        <div class="modal-dialog">
            <form id="editGroupForm" method="POST">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title fw-700">Edit Supplier Group</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
    ...
    <script>
        ...
        $(function() {
            $('.btn-edit-group').on('click', function() {
                var id = $(this).data('id');
                var baseUrl = '{{ url('admin/supplier-groups') }}';
