...
@section('page-title', __("Factories"))
...
@section('page-actions')
  ...
  <a href="{{ route('manufacturing.factories.create') }}" class="bp-btn bp-btn-primary">
...
@section('content')
    ...
    <div class="col-xl-4 col-sm-6">
      <div class="bp-stat-card">
        ...
        <div class="bp-stat-content">
          <div class="bp-stat-label">Total Factories</div>
    ...
    <div class="col-xl-4 col-sm-6">
      <div class="bp-stat-card">
        ...
        <div class="bp-stat-content">
          ...
          <div class="bp-stat-value">{{ $stats['inactive'] ?? 0 }}</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <x-core::table>
    <x-slot:filters>
      <x-core::table.filter-bar searchPlaceholder="Search factory name, contact, phone...">
    ...
    <x-core::table.header>
      ...
      <x-core::table.column>Payment Terms</x-core::table.column>
    ...
    <tbody>
      ...
      <tr>
        <td class="fw-600 fs-12">{{ $factory->code }}</td>
        <td class="fw-700 fs-13">{{ $factory->name }}</td>
        <td>{{ $factory->contact_person ?? '' }}</td>
        <td>{{ $factory->phone ?? '' }}</td>
        <td>{{ $factory->payment_terms ?? '' }}</td>
        <td class="text-end {{ $factory->due_balance > 0 ? 'text-danger fw-700' : '' }}">{{ currency_symbol() }} {{ number_format($factory->due_balance, 0) }}</td>
        <td>
          <x-core::status-toggle :url="route('manufacturing.factories.toggle-status', $factory->id)" :active="$factory->is_active" />
        </td>
        <td>
          ...
          <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
              ...
              <li><a class="dropdown-item" href="{{ route('manufacturing.factories.show', $factory) }}"><i class="fa-solid fa-eye"></i> View</a></li>
              @endbpCan
              @bpCan('manufacturing.edit')
              <li><a class="dropdown-item" href="{{ route('manufacturing.factories.edit', $factory) }}"><i class="fa-solid fa-pen"></i> Edit</a></li>
              @endbpCan
              ...
              <li>
                <form action="{{ route('manufacturing.factories.destroy', $factory) }}" method="POST" class="d-inline">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="dropdown-item text-danger" onclick="return confirm('Are you sure you want to delete {{ $factory->name }}?')"><i class="fa-solid fa-trash"></i> Delete</button>
                </form>
              </li>
              @endbpCan
            </ul>
      ...
      @empty
      <x-core::table.empty colspan="8" icon="fa-solid fa-industry" title="No factories found." />
      @endforelse
    </tbody>

    <x-slot:pagination>
      <x-core::table.pagination :paginator="$factories" itemLabel="factories" />
    </x-slot:pagination>
  </x-core::table>
