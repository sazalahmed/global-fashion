...
@section('title', __("Create Blog Post — Website"))
...
<form action="{{ route('ecommerce.blog.store') }}" method="POST" enctype="multipart/form-data">
  @csrf
        ...
        <div class="bp-card-body">
          <div class="row g-3">
            <div class="col-12">
              ...
              <input type="text" class="bp-form-control" name="title" value="{{ old('title') }}" placeholder="Enter post title" required>
              @error('title')
            ...
            <div class="col-12">
              ...
              <textarea class="bp-form-control" name="excerpt" rows="2" placeholder="Brief summary of the post">{{ old('excerpt') }}</textarea>
              @error('excerpt')
                <div class="text-danger fs-12 mt-1">{{ $message }}</div>
              @enderror
            </div>
            <div class="col-12">
              ...
              <textarea class="bp-form-control bp-richtext" name="content" rows="12" placeholder="Write your post content here...">{{ old('content') }}</textarea>
              @error('content')
                <div class="text-danger fs-12 mt-1">{{ $message }}</div>
              @enderror
            </div>
        ...
        <div class="bp-card-body">
          <div class="row g-3">
            <div class="col-12">
              ...
              <input type="text" class="bp-form-control @error('seo_title') is-invalid @enderror" name="seo_title" data-seo-title value="{{ old('seo_title') }}" placeholder="SEO page title (defaults to post title)">
            ...
            <div class="col-12">
              ...
              <textarea class="bp-form-control @error('seo_description') is-invalid @enderror" name="seo_description" data-seo-desc rows="2" placeholder="Short description for search engines (max 160 chars)">{{ old('seo_description') }}</textarea>
          ...
          <div class="row g-3">
            ...
            <div class="col-12">
              ...
              <select class="bp-form-select w-100" name="show_on_homepage">
                <option value="0" {{ old('show_on_homepage', '0') == '0' ? 'selected' : '' }}>No</option>
                <option value="1" {{ old('show_on_homepage') == '1' ? 'selected' : '' }}>Yes</option>
              </select>
            </div>
            <div class="col-12">
              <label class="bp-form-label">Publish Date</label>
              <input type="datetime-local" class="bp-form-control" name="published_at" value="{{ old('published_at', now()->format('Y-m-d\TH:i')) }}">
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="bp-card-body">
          <div class="row g-3">
            ...
            <div class="col-12">
              ...
              <input type="text" class="bp-form-control" name="tags" data-tagify value="{{ old('tags') }}" placeholder="Type a tag and press Enter">
              <div class="fs-11 text-muted mt-1">e.g. fashion, summer, tips</div>
            </div>
          </div>
        </div>
      </div>
