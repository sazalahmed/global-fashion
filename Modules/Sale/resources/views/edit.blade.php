...
@section('content')
    ...
    <div class="bp-sale-form-wrapper">
        ...
        @php
            ...
            $advanceRows = collect(old('payments'))->isNotEmpty()
                ? collect(old('payments'))->values()
                : $payments
                    ->map(
                        fn($p) => [
                            'id' => $p->id,
                    ...
                    ->values();
            $multiPay = $advanceRows->count() > 1;
        @endphp

        <form action="{{ route('sales.update', $sale) }}" method="POST" id="editInvoiceForm">
            @csrf
            @method('PUT')
            ...
            <div class="bp-card mb-4">
                <div class="bp-card-header">
                    ...
                    <span class="bp-badge bp-badge-info">{{ $sale->invoice_number }}</span>
                </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <input type="date" class="bp-form-control" name="invoice_date"
                                value="{{ old('invoice_date', $sale->sale_date?->format('Y-m-d')) }}" required>
                            @error('invoice_date')
                                <div class="text-danger fs-12 mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-4">
                            @php $currentCustomerId = old('customer_id', $sale->customer_id); @endphp
                            ...
                            <select class="bp-form-select w-100 select2-search" name="customer_id" id="customerSelect">
                                <option value="" {{ $currentCustomerId === null ? 'selected' : '' }}>
                                    {{ __('Walk-in Customer') }}</option>
                                @foreach ($customers as $customer)
                                    <option value="{{ $customer->id }}" data-name="{{ $customer->name }}"
                                        ...
                                        {{ (int) $currentCustomerId === $customer->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="bp-form-select w-100" name="price_type">
                                <option value="regular"
                                    {{ old('price_type', $sale->price_type ?? 'regular') === 'regular' ? 'selected' : '' }}>
                                    {{ __('Regular Price') }}</option>
                                <option value="wholesale"
                                    {{ old('price_type', $sale->price_type) === 'wholesale' ? 'selected' : '' }}>
                                    {{ __('Wholesale Price') }}</option>
                                <option value="resell"
                                    {{ old('price_type', $sale->price_type) === 'resell' ? 'selected' : '' }}>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="bp-form-control" name="customer_name_snapshot"
                                        id="recipientName"
                                        value="{{ old('customer_name_snapshot', $sale->customer_name_snapshot) }}"
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="bp-form-control" name="customer_phone_snapshot"
                                        id="recipientPhone"
                                        value="{{ old('customer_phone_snapshot', $sale->customer_phone_snapshot) }}"
                                ...
                                <div class="col-md-6">
                                    ...
                                    <textarea class="bp-form-control" name="customer_address" id="customerAddress" rows="3"
                                        placeholder="{{ __('Delivery address') }}">{{ old('customer_address', $sale->customer_address) }}</textarea>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="input-group">
                                <select class="bp-form-select bp-discount-type-addon" name="discount_type"
                                    ...
                                    <option value="percentage"
                                        {{ old('discount_type', $sale->discount_type) === 'percentage' ? 'selected' : '' }}>
                                        Percentage (%)</option>
                                </select>
                                <input type="number" class="bp-form-control" name="discount_value"
                                    id="orderDiscountValue"
                                    value="{{ old('discount_value', num_input($sale->discount_value)) }}" step="0.01"
                                    min="0">
                            </div>
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="number" class="bp-form-control" name="shipping_charge" id="shippingCharge"
                                value="{{ old('shipping_charge', num_input($sale->shipping_charge)) }}" step="0.01"
                                min="0">
                        </div>
                        <div class="col-md-4">
                            <label class="bp-form-label">{{ __('Sale Status') }} *</label>
                            {{-- Delivered is final: the select is locked (a disabled
                                 control is not submitted, so the status is kept). --}}
                            <select class="bp-form-select w-100" name="sale_status" id="saleStatus"
                                @if ($sale->status === 'delivered') disabled
                                    title="{{ __('Delivered sales cannot be changed — use Sale Return.') }}"
                                @else required @endif>
                                @foreach ($saleStatuses as $val => $label)
                                    <option value="{{ $val }}"
                                        {{ old('sale_status', $sale->status) === $val ? 'selected' : '' }}>
                                        {{ __($label) }}</option>
                                @endforeach
                            </select>
                            @if ($sale->status === 'delivered')
                                <div class="fs-12 text-muted mt-1">
                                    {{ __('Delivered sales are locked. Use Sale Return to reverse.') }}</div>
                            @endif
                        </div>
                    </div>
                    <div class="row mt-1 g-3">
                            ...
                            <select class="bp-form-select w-100" name="district_id" id="districtSelect"
                                ...
                                @foreach ($districts as $district)
                                    <option value="{{ $district->id }}"
                                        {{ (string) old('district_id', $sale->district_id) === (string) $district->id ? 'selected' : '' }}>
                            ...
                            <select class="bp-form-select w-100" name="thana_id" id="thanaSelect">
                                ...
                                @foreach ($thanas as $thana)
                                    <option value="{{ $thana->id }}"
                                        {{ (string) old('thana_id', $sale->thana_id) === (string) $thana->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <textarea class="bp-form-control" name="notes" rows="3"
                                placeholder="{{ __('Customer-facing note (sent to courier)') }}">{{ old('notes', $sale->notes) }}</textarea>
                        </div>
                        <div class="col-md-6">
                            <label class="bp-form-label">{{ __('Item Description') }}</label>
                            <textarea class="bp-form-control" name="item_description" rows="3"
                                placeholder="{{ __('Parcel contents sent to courier (auto-filled from items if left blank)') }}">{{ old('item_description', $sale->item_description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="row g-3 align-items-start">
                        <div class="col-lg-8">
                            <div id="salePaymentRows">
                                @forelse ($advanceRows as $i => $row)
                                    <div class="roe bp-split-payment-row" data-index="{{ $i }}">
                                        <input type="hidden" name="payments[{{ $i }}][id]"
                                            value="{{ $row['id'] ?? '' }}">
                                        <input type="hidden" name="payments[{{ $i }}][date]"
                                            value="{{ $row['date'] ?? date('Y-m-d') }}">
                                        <input type="hidden" name="payments[{{ $i }}][note]"
                                            value="{{ $row['note'] ?? '' }}">
                                        <div class="bp-pay-amount-col">
                                            <input type="number" class="bp-form-control sale-pay-amount"
                                                name="payments[{{ $i }}][amount]"
                                                placeholder="{{ __('Amount') }}" step="0.01" min="0"
                                                value="{{ $row['amount'] ?? '' }}">
                                        </div>
                                        <div class="bp-pay-method-col">
                                            <select class="bp-form-select sale-pay-account"
                                                name="payments[{{ $i }}][payment_account_id]">
                                                <option value="">{{ __('Select Account') }}</option>
                                                @foreach ($paymentAccounts as $pa)
                                                    <option value="{{ $pa->id }}"
                                                        data-type="{{ $pa->account_type }}"
                                                        {{ (string) ($row['payment_account_id'] ?? '') === (string) $pa->id ? 'selected' : '' }}>
                                                        {{ $pa->name }}</option>
                                                @endforeach
                                                {{-- Accounts since deleted/deactivated — kept so the
                                                     payment's original account stays visible/selected --}}
                                                @foreach ($historicalAccounts as $ha)
                                                    <option value="{{ $ha->id }}"
                                                        data-type="{{ $ha->account_type }}"
                                                        {{ (string) ($row['payment_account_id'] ?? '') === (string) $ha->id ? 'selected' : '' }}>
                                                        {{ $ha->name }} ({{ __('deleted') }})</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <button type="button" class="remove-split {{ $multiPay ? '' : 'd-none' }}"
                                            title="{{ __('Remove') }}"><i class="fa-solid fa-times"></i></button>
                                    </div>
                                @empty
                                    <div class="roe bp-split-payment-row" data-index="0">
                                        <input type="hidden" name="payments[0][id]" value="">
                                        <input type="hidden" name="payments[0][date]" value="{{ date('Y-m-d') }}">
                                        <input type="hidden" name="payments[0][note]" value="">
                                        <div class="bp-pay-amount-col">
                                            <input type="number" class="bp-form-control sale-pay-amount"
                                                name="payments[0][amount]" placeholder="{{ __('Amount') }}"
                                                step="0.01" min="0">
                                        </div>
                                        <div class="bp-pay-method-col">
                                            <select class="bp-form-select sale-pay-account"
                                        ...
                                        <button type="button" class="remove-split d-none"
                                            title="{{ __('Remove') }}"><i class="fa-solid fa-times"></i></button>
                                    </div>
                                @endforelse
                            </div>
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center">
                @bpCan('sales.delete')
                    <button type="button" class="bp-btn bp-btn-danger" id="deleteInvoiceBtn">
                        <i class="fa-solid fa-trash me-1"></i> {{ __('Delete Sale') }}
                    </button>
                @endbpCan
                <div class="d-flex gap-2">
                    <a href="{{ route('sales.index') }}" class="bp-btn bp-btn-danger"><i class="fa-solid fa-xmark"></i>
                        {{ __('Cancel') }}</a>
                    <button type="submit" class="bp-btn bp-btn-success" name="action" value="update"><i
                            class="fa-solid fa-check me-1"></i> {{ __('Update Sale') }}</button>
                </div>
            </div>
        ...
        $productCatalogJson = $products
            ->map(function ($p) {
                return [
                    ...
                    // Storefront discount as a % off sell_price (campaign / flash
                    // deal / own discount, lowest wins). Percentage keeps the line
                    // correct at any quantity. 0 when the product has no discount.
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i
                            class="fa-solid fa-triangle-exclamation text-danger me-2"></i>{{ __('Delete Sale') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <p>{{ __('Are you sure you want to delete sale') }} <strong>{{ $sale->invoice_number }}</strong>?
                        {{ __('This action cannot be undone.') }}</p>
                    <p class="text-muted fs-12">
                        {{ __('This will reverse any stock adjustments and payment records associated with this sale.') }}
                    </p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="bp-btn bp-btn-primary" data-bs-dismiss="modal"><i
                            class="fa-solid fa-xmark me-1"></i>{{ __('Cancel') }}</button>
                    <form action="{{ route('sales.destroy', $sale) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="bp-btn bp-btn-danger"><i class="fa-solid fa-trash me-1"></i>
...
@push('scripts')
    ...
    <script>
        ...
        // Order line items to render on load: existing sale items, or the
        // submitted rows after a validation error.
        ...
        $(function() {
            ...
            function renderComboRow(combo, $replace) {
                ...
                var compHtml = combo.components.map(function(c) {
                    // Drop the baked " — {size}" suffix so the size shows once via
                    // the variant chip — even after the size is edited.
                    var name = stripComboVariantSuffix(c.productName || '');
                ...
                // Newest product on top for interactive adds; prefill (existing
                // items / validation restore) keeps its saved order via appendTo.
            ...
            function cmgrRenderComponents() {
                ...
                comboMgr.components.forEach(function(c, i) {
                    ...
                    // showing it twice (handles an edited size where the suffix is
                    // stale relative to the chosen variant).
                    var pname = stripComboVariantSuffix(c.productName || '');
            ...
            // SaleService stores combo component names with the variant baked in
            // as a " — {size}" suffix (em-dash), while the size also lives in its
            // own variant_label / variant column here. Strip that suffix so the
            // size shows once — and so an edited size (variant column) is never
            // shown alongside a stale baked size. Product names use " - " (hyphen),
            // so the em-dash separator is unambiguously the variant suffix.
            function stripComboVariantSuffix(name) {
                var n = String(name || '');
            ...
            function addLine(d) {
                ...
                // (search, variant picker, barcode, preloaded items) shows it.
                ...
                if (typeof d.discount === 'number') {
                    // Explicit discount passed (existing line, or prefill restore
                    // after a validation error) — honour it as-is, never override.
            ...
            // ── Customer's previous due (display-only; excludes this sale) ──
            ...
            function recalculate() {
                ...
                $('#orderTable tbody tr').each(function() {
                    ...
                    // Match the storefront (Product::displayPrice) and the create
                    // page: round the after-discount UNIT price UP to a whole BDT,
                    // so subtotals are clean integers (e.g. 900 − 27.78% = 649.98
                    // → 650). Back out the actual discount from the rounded
                    // subtotal so the posted discount and the displayed subtotal
                    // always agree.
                    var lineSub, disc;
                    if (qty > 0) {
            ...
            // per variant. The line discount goes on the first variant so the line
            // total is unchanged.
            function expandOrderItems() {
                ...
                $('#editInvoiceForm').append($hold);
                return k;
            }

            // ── Advance (split payment) logic ──
            var salePayIndex = {{ $advanceRows->count() ?: 1 }};
            ...
            $('#saleAddPayment').on('click', function() {
                ...
                var html = '<div class="bp-split-payment-row" data-index="' + idx + '">' +
                    '<input type="hidden" name="payments[' + idx + '][id]" value="">' +
                    '<input type="hidden" name="payments[' + idx +
            ...
            $('#deleteInvoiceBtn').on('click', function() {
                var modal = new bootstrap.Modal(document.getElementById('deleteInvoiceModal'));
                modal.show();
            });
            ...
            var oldThanaId = '{{ old('thana_id', $sale->thana_id) }}';
            ...
            // Reload thanas for the saved district and keep the saved thana selected.
            if ($districtSelect.val()) loadThanas($districtSelect.val(), oldThanaId);

            // Render the order rows (existing items, or submitted rows after error)
            // — group the flat per-variant items back into consolidated rows.
            ...
            function renderPrefillItems(items) {
                ...
                // Combo items: regroup the flattened component items back into a
                // single combo row per combo_group (price = stored combo price).
                ...
                if (comboItems.length) {
                    ...
                    corder.forEach(function(g) {
                        ...
                        // total on THIS invoice (Σ price×qty − discount), not the
                        // catalog combo_price — legacy orders may carry a different
                        // total (e.g. multiple combos), and editing must not silently
                ...
                order.forEach(function(key) {
                    ...
                    if (isVariable) {
                        ...
                        addLine({
                            productId: first.id,
                            // Stored product_name bakes the first variant in as a
                            // " — {size}" suffix; the variants column lists every
                            // size, so drop the suffix from the title.
                            productName: stripComboVariantSuffix(first.name),
                            image: first.image || '',
                            variantId: '',
                            variantLabel: 'Multiple',
                            quantity: totalQty,
