...
@section('title', __('Create Combo — Website'))
@section('page-title', __('Create Combo'))
...
@section('content')
    ...
    @php
        ...
        // Build a JSON catalog for the JS: variant dependency, gallery refresh, price preview.
        $catalog = $products
        ...
        // Repopulate rows on validation failure.
        $oldItems = old('items', [['product_id' => '', 'variant_id' => '', 'quantity' => 1]]);
        $oldGallery = old('gallery', []);
    @endphp

    <form action="{{ route('products.combos.store') }}" method="POST" enctype="multipart/form-data" id="comboForm">
        @csrf
                        ...
                        <div class="row g-3">
                            <div class="col-md-8">
                                ...
                                <input type="text" class="bp-form-control" name="name" value="{{ old('name') }}"
                                    placeholder="e.g. Weekend Family Bundle" required>
                                @error('name')
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="bp-form-control bp-richtext" name="description" rows="3"
                                    placeholder="Short description of this combo">{{ old('description') }}</textarea>
                                @error('description')
                                    <div class="text-danger fs-12 mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-12">
                                <x-core::image-upload name="thumbnail" label="Thumbnail"
                                    accept="image/png,image/jpeg,image/webp" hint="JPG, PNG or WEBP, max 2MB." />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-xl-4">
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <input type="number" class="bp-form-control" name="combo_price" id="comboPrice"
                                    value="{{ old('combo_price', 0) }}" min="0" step="0.01" required>
                                @error('combo_price')
                                    <div class="text-danger fs-12 mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="bp-btn bp-btn-success justify-content-center">
                        <i class="fa-solid fa-save me-2"></i> Create Combo
                    </button>
                </div>
...
@push('scripts')
    <script>
        ...
        $(function() {
            ...
            var oldItems = @json(array_values($oldItems));
            var oldGallery = @json(array_values($oldGallery));
